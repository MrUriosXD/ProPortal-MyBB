...
if(!$mybb->input['action'])
{
	...
	$page->output_header($lang->blog_block_management);
	...
	if(!empty($mybb->input['position']) && is_array($mybb->input['position']))
	{
		...
		{
			$db->update_query("blog_blocks", array('position' => intval($order)), "id='".intval($update_id)."'");
		}

		$plugins->run_hooks("admin_blog_blocks_start_position_commit");
	...
	{
		$table = new Table;
		$table->construct_header($lang->blog_blocks, array('width' => '100%'));

		$table->construct_cell($lang->blog_blocks_noblock);
		$table->construct_row();
		...
		$table->construct_header($lang->blog_blocks_zone, array('width' => '10%'));
		$table->construct_header($lang->blog_blocks_position, array('width' => '10%'));
		$table->construct_header($lang->blog_blocks_custom_short, array('width' => '10%'));
		$table->construct_header($lang->blog_blocks_file, array('width' => '15%'));
		$table->construct_header($lang->blog_blocks_isenabled, array('width' => '10%'));
		$table->construct_header($lang->blog_blocks_visible_s, array('width' => '10%'));
		$table->construct_header($lang->blog_blocks_options, array('width' => '10%'));

		$query = $db->simple_select("blog_blocks", "*", "", array('order_by' => 'zone, position'));
		while($block = $db->fetch_array($query))
		{
			...
			elseif($block['zone'] == "2"){ $zone = $lang->blog_blocks_zoneright; }

			if($block['custom'] == "0"){ $custom = $lang->blog_blocks_no; }
			elseif($block['custom'] == "1"){ $custom = $lang->blog_blocks_yes; }

			if($block['enabled'] == "0"){ $enabled = "<span style=\"color:red;\">".$lang->blog_blocks_no."</span>"; }
			elseif($block['enabled'] == "1"){ $enabled = "<span style=\"color:green;\">".$lang->blog_blocks_yes."</span>"; }

			if($block['file'] == "0"){ $file = $lang->blog_blocks_nofile; }
			...
			$popup = new PopupMenu("group_{$block['id']}", $lang->blog_blocks_seegroup);
		...
		$buttons[] = $form->generate_submit_button($lang->blog_blocks_updateposition);
...
if($mybb->input['action'] == "add")
{
	...
	if($mybb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$db->insert_query("blog_blocks", $insert_array);
	...
	$block_file = array();
	$block_file[] = $lang->blog_blocks_nofile;
	unset($filearray);
	$filearray = array();
	...
	$form_container->output_row($lang->blog_blocks_zone, "", $form->generate_select_box('zone', $block_zone, $block_data['zone'], array('checked' => $block_data['zone'], 'id' => 'zone')), 'zone');
	...
	$form_container->output_row($lang->blog_blocks_custom, $lang->blog_blocks_custom_desc, $form->generate_radio_button('custom', '1', $lang->blog_blocks_yes)."<br />\n".$form->generate_radio_button('custom', '0', $lang->blog_blocks_no));
	$form_container->output_row($lang->blog_blocks_file, $lang->blog_blocks_file_desc, $form->generate_select_box('file', $block_file, $block_data['file'], array('checked' => $block_data['file'], 'id' => 'file" onChange="return checksel()')), 'file');
	...
	$form_container->output_row($lang->blog_blocks_enabled, "", $form->generate_radio_button('enabled', '1', $lang->blog_blocks_yes)."<br />\n".$form->generate_radio_button('enabled', '0', $lang->blog_blocks_no));
	$form_container->output_row($lang->blog_blocks_visible, $lang->blog_blocks_visible_desc, $form->generate_select_box('visible[]', $usergroups, $block_data['visible'], array('multiple' => 'multiple', 'id' => 'visible')), 'visible');

	$form_container->end();

	$buttons[] = $form->generate_submit_button($lang->blog_blocks_add);
	$form->output_submit_wrapper($buttons);
	$form->end();
...
if($mybb->input['action'] == "edit")
{
	...
	if($mybb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$db->update_query("blog_blocks", $update_array, "id='{$id}'");
	...
	$form_container->output_row($lang->blog_blocks_zone, "", $form->generate_select_box('zone', $block_zone, $block_data['zone'], array('checked' => $block_data['zone'], 'id' => 'zone')), 'zone');
	$form_container->output_row($lang->blog_blocks_position, "", $form->generate_text_box('position', $block_data['position'], array('id' => 'position')), 'position');
	$form_container->output_row($lang->blog_blocks_custom, $lang->blog_blocks_custom_desc, $form->generate_radio_button('custom', '1', $lang->blog_blocks_yes, $blockcustom_yes)."<br />\n".$form->generate_radio_button('custom', '0', $lang->blog_blocks_no, $blockcustom_no));
	$form_container->output_row($lang->blog_blocks_file, $lang->blog_blocks_file_desc, $form->generate_select_box('file', $block_file, $block_data['file'], array('checked' => $block_data['file'], 'id' => 'file" onChange="return checksel()')), 'file');
	$form_container->output_row($lang->blog_blocks_content, $lang->blog_blocks_content_desc, $form->generate_text_area('content', $block_data['content'], array('id' => 'bcontent', 'style' => 'width: 450px; height: 250px;')), 'bcontent');
	$form_container->output_row($lang->blog_blocks_enabled, "", $form->generate_radio_button('enabled', '1', $lang->blog_blocks_yes, $blockenabled_yes)."<br />\n".$form->generate_radio_button('enabled', '0', $lang->blog_blocks_no, $blockenabled_no));
	$form_container->output_row($lang->blog_blocks_visible, $lang->blog_blocks_visible_desc, $form->generate_select_box('visible[]', $usergroups, $block_data['visible'], array('checked' => $block_data['visible'], 'multiple' => 'multiple', 'id' => 'visible')), 'visible');

	$form_container->end();

	$buttons[] = $form->generate_submit_button($lang->blog_blocks_edit);
	$form->output_submit_wrapper($buttons);
	$form->end();
...
if($mybb->input['action'] == "delete")
{
	...
	$db->delete_query("blog_blocks", "id='{$id}'");
	$plugins->run_hooks("admin_blog_blocks_delete_commit");
...
{
	$plugins->run_hooks("admin_blog_blocks_enable");
	...
	$plugins->run_hooks("admin_blog_blocks_enable_commit");
...
{
	$plugins->run_hooks("admin_blog_blocks_disable");
	...
	$plugins->run_hooks("admin_blog_blocks_disable_commit");
