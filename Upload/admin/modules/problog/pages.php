...
$plugins->run_hooks("admin_blog_pages_begin");

if(!$mybb->input['action'])
{
	$plugins->run_hooks("admin_blog_pages_start");

	$page->output_header($lang->blog_page_management);

	$page->output_nav_tabs($sub_tabs, 'pages');

	$query = $db->simple_select("blog_pages", "*");
	$pages = $db->fetch_array($query);

	if(!$pages)
	{
		$table = new Table;
		$table->construct_header($lang->blog_pages, array('width' => '100%'));

		$table->construct_cell($lang->blog_pages_nopage);
		$table->construct_row();

		$table->output($lang->blog_manage_pages);
	} else {
		$table = new Table;
		$table->construct_header($lang->blog_pages_title, array('width' => '35%'));
		$table->construct_header($lang->blog_pages_name, array('width' => '20%'));
		$table->construct_header($lang->blog_pages_isenabled, array('width' => '15%'));
		$table->construct_header($lang->blog_pages_visible_s, array('width' => '15%'));
		$table->construct_header($lang->blog_pages_options, array('width' => '15%'));

		$query = $db->simple_select("blog_pages", "*", "", array('order_by' => 'id'));
		while($mypage = $db->fetch_array($query))
		{
			if($mypage['enabled'] == "0"){ $enabled = "<span style=\"color:red;\">".$lang->blog_pages_no."</span>"; }
			elseif($mypage['enabled'] == "1"){ $enabled = "<span style=\"color:green;\">".$lang->blog_pages_yes."</span>"; }
			...
			$popup = new PopupMenu("group_{$mypage['id']}", $lang->blog_pages_seegroup);
...
{
	$plugins->run_hooks("admin_blog_pages_add");

	if($mybb->request_method == "post")
	{
		$checkquery = $db->simple_select('blog_pages','*','name LIKE "'.$db->escape_string(trim($mybb->input['name'])).'"');
		$check = $db->fetch_array($checkquery);

		if($check)
		{
			$errors[] = $lang->blog_pages_check;
		}

		if(!trim($mybb->input['title']))
		{
			$errors[] = $lang->blog_pages_no_title;
		}

		if(!trim($mybb->input['name']))
		{
			$errors[] = $lang->blog_pages_no_name;
		}

		if(!trim($mybb->input['content']))
		{
			$errors[] = $lang->blog_pages_no_content;
		}
		...
		if(!$errors)
		{
			...
			$db->insert_query("blog_pages", $insert_array);

			$plugins->run_hooks("admin_blog_pages_add_commit");
	...
	$page->add_breadcrumb_item($lang->blog_pages_add);
	...
	if($problog->settings['showeditor'] == "1")
	{
		echo'<script src="http://js.nicedit.com/nicEdit-latest.js" type="text/javascript"></script>
<script type="text/javascript">bkLib.onDomLoaded(nicEditors.allTextAreas);</script>';
	}

	$form_container = new FormContainer($lang->blog_pages_add);
	$form_container->output_row($lang->blog_pages_title." <em>*</em>", "", $form->generate_text_box('title', $page_data['title'], array('id' => 'title')), 'title');
	$form_container->output_row($lang->blog_pages_name." <em>*</em>", $lang->blog_pages_name_desc, $form->generate_text_box('name', $page_data['name'], array('id' => 'name')), 'name');
	$form_container->output_row($lang->blog_pages_content." <em>*</em>", "", $form->generate_text_area('content', $page_data['content'], array('id' => 'pcontent', 'style' => 'width: 650px; height: 350px;')), 'pcontent');
	$form_container->output_row($lang->blog_pages_enabled, "", $form->generate_radio_button('enabled', '1', $lang->blog_pages_yes, $pageenabled_yes)."<br />\n".$form->generate_radio_button('enabled', '0', $lang->blog_pages_no, $pageenabled_no));
	$form_container->output_row($lang->blog_pages_visible, $lang->blog_pages_visible_desc, $form->generate_select_box('visible[]', $usergroups, $page_data['visible'], array('multiple' => 'multiple', 'id' => 'visible')), 'visible');

	$form_container->end();

	$buttons[] = $form->generate_submit_button($lang->blog_pages_add);
...
if($mybb->input['action'] == "edit")
{
	$plugins->run_hooks("admin_blog_pages_edit");
	...
	if($mybb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$db->update_query("blog_pages", $update_array, "id='{$id}'");

			$plugins->run_hooks("admin_blog_pages_edit_commit");
	...
	$page->add_breadcrumb_item($lang->blog_pages_edit);
...
<script type="text/javascript">bkLib.onDomLoaded(nicEditors.allTextAreas);</script>';
	...
	$form_container = new FormContainer($lang->blog_pages_edit);
	$form_container->output_row($lang->blog_pages_title." <em>*</em>", "", $form->generate_text_box('title', $page_data['title'], array('id' => 'title')), 'title');
	$form_container->output_row($lang->blog_pages_name." <em>*</em>", $lang->blog_pages_name_desc, $form->generate_text_box('name', $page_data['name'], array('id' => 'name')), 'name');
	$form_container->output_row($lang->blog_pages_content." <em>*</em>", "", $form->generate_text_area('content', $page_data['content'], array('id' => 'pcontent', 'style' => 'width: 650px; height: 350px;')), 'pcontent');
	$form_container->output_row($lang->blog_pages_enabled, "", $form->generate_radio_button('enabled', '1', $lang->blog_pages_yes, $pageenabled_yes)."<br />\n".$form->generate_radio_button('enabled', '0', $lang->blog_pages_no, $pageenabled_no));
	...
	$buttons[] = $form->generate_submit_button($lang->blog_pages_edit);
	$form->output_submit_wrapper($buttons);
	$form->end();
...
if($mybb->input['action'] == "delete")
{
	...
	$db->delete_query("blog_pages", "id='{$id}'");
	$plugins->run_hooks("admin_blog_pages_delete_commit");
...
{
	$plugins->run_hooks("admin_blog_pages_enable");
	...
	$plugins->run_hooks("admin_blog_pages_enable_commit");
...
{
	$plugins->run_hooks("admin_blog_pages_disable");
	...
	$plugins->run_hooks("admin_blog_pages_disable_commit");
