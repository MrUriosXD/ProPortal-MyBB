...
$announcementsfids = explode(',', $problog->settings['announcementsfid']);
...
$annnum = $db->fetch_field($db->simple_select('threads', 'COUNT(*) AS annnum', "fid IN (".$problog->settings['announcementsfid'].") AND visible='1' AND closed NOT LIKE 'moved|%'"), 'annnum');
$pagenum = (int)($mybb->input['page'] ?? 1);
$totalpage = (int)ceil($annnum / (int)$problog->settings['numannouncements']);
if($pagenum < 1 || !$pagenum || $pagenum > $totalpage){ $pagenum = 1; }
$multipage = multipage($annnum, $problog->settings['numannouncements'], $pagenum, $mybb->settings['bburl'].'/blog.php');
...
$query = $db->query("
	...
	LIMIT ".(($pagenum-1)*$problog->settings['numannouncements']).", ".$problog->settings['numannouncements']
);
...
$query = $db->query("
	...
	ORDER BY t.dateline DESC
	LIMIT 0, ".$problog->settings['numannouncements']
);
while($announcement = $db->fetch_array($query))
{
	...
	if($problog->settings['annmessagelength'] > "0")
	{
		$message = substr($message, 0, $problog->settings['annmessagelength']);
