...
if (!defined("IN_BLOG")) {
	die("<div style=\"border:1px solid #CC0000; padding:3px; margin:0; font-family:Tahoma; width:250px; font-size:12px;\"><strong>Uyar\xfd:</strong> Bu dosyay\xfd g\xf6r\xfcnt\xfcleme izniniz yok.</div>");
}
...
if($mybb->user['uid'] != 0)
{
	...
	{ // if there aren't any new posts, there is no point in wasting two more queries
		...
		$query = $db->simple_select("threads", "COUNT(tid) AS newann", "dateline>'".$mybb->user['lastvisit']."' AND fid IN (".$problog->settings['announcementsfid'].") $unviewwhere");
	...
	if($mybb->user['avatar']){ $mybb->user['avatar'] = htmlspecialchars_uni($mybb->user['avatar']); }else{ $mybb->user['avatar'] = $mybb->settings['bburl']."/blog/images/user.png"; }
...
else
{
	...
	<br />
	<form method=\"post\" action=\"{$blog_url}\"><input type=\"hidden\" name=\"action\" value=\"do_login\" />
