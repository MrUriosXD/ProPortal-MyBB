...
$lang = new MyLanguage();
$lang->set_path(MYBB_ROOT.'blog/install/resources');
$lang->load('language');
...
// Version information of ProBlog
$blog_version = array(
	"version" => "1.0 Beta",
	"version_code" => "10B",
);
...
elseif(file_exists('installed') && !$mybb->input['action'])
{
	...
	echo $lang->problog_already_installed;
...
function intro()
{
	global $output, $mybb, $lang, $blog_version;
	...
	echo $lang->sprintf($lang->welcome_step, $blog_version['version']);
...
function requirements_check()
{
	...
	if(!file_exists(MYBB_ROOT.'blog/inc/blog.class.php'))
	{
...
function insert_templates()
{
	...
	$contents = @file_get_contents(INSTALL_ROOT.'resources/problog_templates.xml');
...
function uninstall()
{
	...
	$tables = array("blog_blocks","blog_pages","blog_settings");
	...
	if($table_count > 0)
	{
		...
		$templates = array("pro_blog","pro_blog_announcement","pro_blog_block","pro_blog_left","pro_blog_page","pro_blog_right");
